...
uploadSuzanne(Device* device, CmdBuffer* cmdBuffer)
{
	...
	cmdBuffer->BufferBarrier({ UsageScope::None,
							   UsageScope::VertexBuffer,
							   result.vbuf });
	cmdBuffer->BufferBarrier(
		{ UsageScope::None, UsageScope::IndexBuffer, result.ibuf });
...
main()
{
	...
	while (true) {
		...
		Semaphore available;
		if (!device->AquireNext(swapchain, &swapchainTexture, &available)) {
		...
		cmdBuffer->TextureBarrier({ UsageScope::None,
									UsageScope::ColorAttachment,
									multiSampleTexture });
		cmdBuffer->TextureBarrier({ UsageScope::None,
									UsageScope::ColorAttachment,
									swapchainTexture });
		...
		cmdBuffer->TextureBarrier(
			{ UsageScope::ColorAttachment, // not sure what the resolve
		...
		cmdBuffer->TextureBarrier({ UsageScope::ColorAttachment,
		...
		if (!device->Present(swapchain, &renderDone)) {
			device->Release(swapchain);
