...
FNVHash hash(void const *data, uint32_t size, FNVHash prev = {})
{
...
FNVHash hash(RenderPassInternal const *renderpass, FNVHash prev = {})
{
...
FNVHash hash(Framebuffer const *framebuffer, FNVHash prev = {})
{
...
FNVHash hash(PipelineLayout const *pipelineLayout, FNVHash prev = {})
{
...
FNVHash hash(DescriptorSetLayout const *layout, FNVHash prev = {})
{
	return hash(&layout->bindings,
				sizeof(layout->bindings[0]) * layout->bindingCnt, prev);
}
...
bool Shared::getPipelineLayout(PipelineLayout const *in,
...
bool Shared::getShaderModule(char const *filepath, VkShaderModule *out0,
							 ShaderLayout *out1)
{
...
bool Shared::getDescriptorSetLayout(DescriptorSetLayout const *in,
...
bool Shared::getGraphicsPipeline(char const *vert, char const *frag,
								 GraphicsPipelineState const *state,
								 ...
								 Pipeline *out)
{
	...
	auto pipeline = createGraphicsPipeline(
		device, modules, stages, 2, vkPipelineLayout, state, renderPass, subpass);
	graphicsPipelines[h] = {pipeline, vkPipelineLayout, pipelineLayout};
	*out = {pipeline, vkPipelineLayout, pipelineLayout};
	return true;
}

bool DescriptorSetAllocator::getDescriptorSet(VkDescriptorSetLayout handle,
											  ...
											  VkDescriptorSet *out)
{
	...
	if (it != free.end())
	{
		...
		working[handle].emplace_back(std::move(pool));
		*out = set;
		return true;
	}
	...
	auto createInfo = vkiDescriptorPoolCreateInfo(DescriptorSetPool::MAX_NUM_SETS,
												  poolSizeCnt, poolSizes);
...
VkCommandBuffer
CommandBufferAllocator::getCommandBuffer(uint32_t queueFamilyIdx,
										 uint64_t frame)
{
	...
	if (it != free.end())
	{
		...
		working[queueFamilyIdx].emplace_back(std::move(pool));
		return set;
	}
	...
	auto allocateInfo = vkiCommandBufferAllocateInfo(
		pool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, CommandPool::MAX_NUM_BUFFERS);
