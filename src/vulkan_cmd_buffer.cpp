...
void VulkanCmdBuffer::BufferBarrier(BufferBarrierInfo const &info)
{
  ...
  auto barrier =
      vkiBufferMemoryBarrier(src.accessFlags, dst.accessFlags, -1, -1,
                             bufferInfo->buffer, 0, bufferInfo->size);

  vkCmdPipelineBarrier(cmdBuffer, src.stageFlags, dst.stageFlags,
                       VK_DEPENDENCY_BY_REGION_BIT, 0, nullptr, 1, &barrier, 0,
                       nullptr);
}

void VulkanCmdBuffer::TextureBarrier(TextureBarrierInfo const &info)
{
  auto src = AccessScope::getAccessScope(info.src);
  auto dst = AccessScope::getAccessScope(info.dst);
  auto textureInfo = ((VulkanDevice::TextureInfo *)info.texture);

  auto barrier = vkiImageMemoryBarrier(
      src.accessFlags, dst.accessFlags, src.layout, dst.layout, -1, -1,
      textureInfo->image, textureInfo->subresource);
...
void VulkanCmdBuffer::Copy(Buffer dst, Buffer src)
{
  ...
  vkCmdCopyBuffer(cmdBuffer, srcBufferInfo->buffer, dstBufferInfo->buffer, 1,
                  &pkCopy);
}

void VulkanCmdBuffer::CopyBufferToTexture(Texture dst, Buffer src)
{
  ...
  vkCmdCopyBufferToImage(cmdBuffer, srcBufferInfo->buffer,
                         dstTextureInfo->image,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &pkCopy);
}

void VulkanCmdBuffer::BeginRenderPass(RenderPassBeginInfo const &beginInfo)
{
  cmdBufferState = {};
  ...
  VkRenderPassBeginInfo renderPassBeginInfo =
      vkiRenderPassBeginInfo(cmdBufferState.renderPass, framebuffer, renderArea,
                             rp.attachmentCnt, clearValues);
  vkCmdBeginRenderPass(cmdBuffer, &renderPassBeginInfo,
                       VK_SUBPASS_CONTENTS_INLINE);
}

void VulkanCmdBuffer::BindVertexBuffer(Buffer buffer, uint64_t offset)
{
  auto bufferInfo = (VulkanDevice::BufferInfo *)buffer;
  vkCmdBindVertexBuffers(cmdBuffer, 0, 1, &bufferInfo->buffer, &offset);
}

void VulkanCmdBuffer::BindIndexBuffer(Buffer buffer)
{
  auto bufferInfo = (VulkanDevice::BufferInfo *)buffer;
  vkCmdBindIndexBuffer(cmdBuffer, bufferInfo->buffer, 0,
                       VkIndexType::VK_INDEX_TYPE_UINT32);
}

void VulkanCmdBuffer::BindUniformBuffer(Buffer buffer, uint32_t set,
...
void VulkanCmdBuffer::BindStorageBuffer(Buffer buffer, uint32_t set,
                                        uint32_t binding, uint32_t offset,
                                        uint32_t range)
{
  auto bufferInfo = (VulkanDevice::BufferInfo *)buffer;
...
void VulkanCmdBuffer::BindSampler(uint32_t set, uint32_t binding)
{
...
void VulkanCmdBuffer::BindSampledTexture(Texture texture, uint32_t set,
                                         uint32_t binding)
{
  auto textureInfo = (VulkanDevice::TextureInfo *)texture;
  auto &d = cmdBufferState.sets[set].bindings[binding];
  d.imageInfo =
      vkiDescriptorImageInfo(VK_NULL_HANDLE, textureInfo->view,
                             VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  cmdBufferState.sets[set].dirty = true;
}

void VulkanCmdBuffer::SetPipelineState(PipelineState pipelineState)
{
  Pipeline pipe = {cmdBufferState.renderPass, cmdBufferState.subpass,
                   &pipelineState};
  auto pipeline = GetPipeline(&device->staticResources, &pipe,
...
void VulkanCmdBuffer::flushDescriptorSets(VkPipelineBindPoint bindPoint)
{
  for (int i = 0; i < cmdBufferState.pipelineLayout.layoutCnt; ++i)
  {
    ...
    if (cmdBufferState.sets[i].dirty)
    {
      ...
      for (int j = 0; j < bindingCnt; ++j)
      {
        writes[j] = vkiWriteDescriptorSet(
            ...
            cmdBufferState.pipelineLayout.layouts[i].bindings[j].binding, 0, 1,
            cmdBufferState.pipelineLayout.layouts[i].bindings[j].descriptorType,
            nullptr, nullptr, nullptr);

        switch (cmdBufferState.pipelineLayout.layouts[i]
                    .bindings[j]
                    .descriptorType)
        {
    ...
    vkCmdBindDescriptorSets(cmdBuffer, bindPoint, layout, 0, 1,
                            &cmdBufferState.sets[i].set, dynamicOffsetCnt,
                            dynamicOffsets);
...
void VulkanCmdBuffer::Draw(uint32_t cnt)
{
  flushDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS);
  vkCmdDraw(cmdBuffer, cnt, 1, 0, 0);
}

void VulkanCmdBuffer::DrawIndexed(uint32_t cnt)
{
  flushDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS);
  vkCmdDrawIndexed(cmdBuffer, cnt, 1, 0, 0, 0);
}

void VulkanCmdBuffer::EndRenderPass() { vkCmdEndRenderPass(cmdBuffer); }

void VulkanCmdBuffer::Dispatch(uint32_t x, uint32_t y, uint32_t z)
{
