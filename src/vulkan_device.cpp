...
bool
VulkanDevice::AquireNext(Swapchain swapchain, Texture* texture, Semaphore* signal)
{
	...
	if (signal != nullptr) {
		sem = AllocateSemaphore(&allocator);
		*(VkSemaphore*)(signal) = sem;
	}
...
bool
VulkanDevice::Present(Swapchain swapchain, Semaphore* wait)
{
	auto info = (SwapchainInfo*)swapchain;
	VkResult result;
	...
	if (wait) {
		ReleaseSemaphore(&allocator, *(VkSemaphore*)wait);
	}
...
VulkanDevice::Recycle()
{
	...
	while (!tickets.empty() &&
		vkGetFenceStatus(device, tickets.front().fence) == VK_SUCCESS) {
		ReleaseFence(&allocator, tickets.front().fence);
		tickets.pop();
	}
...
Ticket
VulkanDevice::Submit(CmdBuffer* cmdBuffer_, Semaphore* wait, UsageScope waitScope, Semaphore* signal)
{
	...
	if (signal != nullptr) {
		*(VkSemaphore*)signal = AllocateSemaphore(&allocator);
	}

	auto accessScope = AccessScope::getAccessScope(waitScope);

	auto submitInfo =
		vkiSubmitInfo(
			wait ? 1 : 0,
			(VkSemaphore*)wait,
			&accessScope.stageFlags,
			1,
			&cmdBuffer->cmdBuffer,
			signal ? 1 : 0,
			(VkSemaphore*)signal);
	...
	tickets.push({ ticketCounter, fence });
...
VulkanDevice::Wait(Ticket ticket_)
{
	...
	while (!tickets.empty() && tickets.front().ticket <= ticket) {
		vkWaitForFences(device, 1, &tickets.front().fence, VK_TRUE, -1);
		ReleaseFence(&allocator, tickets.front().fence);
		tickets.pop();
	}
}
