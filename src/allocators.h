...
using HashMap = std::unordered_map<FNVHash, Value, FNVHashIdentityHasher,
                                   FNVHashComparator>;
...
struct Shared
{
  ...
  bool getDescriptorSetLayout(DescriptorSetLayout const *,
                              VkDescriptorSetLayout *);
  bool getSampler(VkSampler *);
  bool getComputePipeline(const char *compute, Pipeline *);
  bool getGraphicsPipeline(const char *vert, const char *frag,
                           GraphicsPipelineState const *, VkRenderPass,
                           uint32_t subpass, Pipeline *);
...
{
  bool getDescriptorSet(VkDescriptorSetLayout, DescriptorSetLayout const *,
                        uint64_t frame, VkDescriptorSet *);
  ...
  using PoolMap =
      std::unordered_map<VkDescriptorSetLayout, std::deque<DescriptorSetPool>>;
