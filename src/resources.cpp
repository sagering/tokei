...
VkRenderPass
createRenderPass(VkDevice device, RenderPassInternal const* renderPass)
{
	auto createInfo = vkiRenderPassCreateInfo(renderPass->attachmentCnt,
		renderPass->attachments,
		renderPass->subpassCnt,
		renderPass->subpassess,
		renderPass->dependencyCnt,
...
VkFramebuffer
createFramebuffer(VkDevice device, Framebuffer const* framebuffer)
{
	auto createInfo = vkiFramebufferCreateInfo(
		framebuffer->renderPass,
		framebuffer->attachmentCnt,
...
createDescriptorSetLayout(VkDevice device, DescriptorSetLayout const* layout) {
	auto createInfo =
		vkiDescriptorSetLayoutCreateInfo(
			layout->bindingCnt, layout->bindings
		);
...
VkPipelineLayout
createPipelineLayout(VkDevice device, PipelineLayout const* layout)
{
...
uint32_t*
load(char const* filepath, uint32_t* size)
{
	uint32_t* code;
...
VkShaderModule
createShaderModule(VkDevice device, char const* filepath)
{
	uint32_t size = 0;
...
bool
reflectLayout(uint32_t const* code, uint32_t size, VkShaderStageFlags stage, ShaderLayout& layout)
{
...
bool
mergeShaderLayouts(ShaderLayout const* layouts,
	uint32_t layoutCnt,
...
PipelineLayout
createPipelineLayout(ShaderLayout * shaderLayouts, VkShaderStageFlagBits const* stageFlags, uint32_t cnt) {
	PipelineLayout ret = {};

	for (auto i = 0; i < cnt; ++i) {
		for (uint32_t bindingIdx = 0;
			bindingIdx < shaderLayouts[i].bindingCnt;
			++bindingIdx) {
			shaderLayouts[i].bindings[bindingIdx].binding.stageFlags =
				stageFlags[i];
		}
	}
...
VkPipeline
createComputePipeline(VkDevice device, VkShaderModule shaderModule, VkPipelineLayout layout) {
	...
	auto createInfo = vkiComputePipelineCreateInfo(
		shaderStageCreateInfo, layout, VK_NULL_HANDLE, 0
	);
...
createGraphicsPipeline(
	...
	for (uint32_t i = 0; i < pipeline->vertexInput.bindingDescriptionCnt;
		++i) {
	...
	for (uint32_t i = 0;
		i < pipeline->vertexInput.attributeDescriptionCnt;
		++i) {
	...
	auto vertexInputState = vkiPipelineVertexInputStateCreateInfo(
		pipeline->vertexInput.bindingDescriptionCnt,
		bindingDescriptions,
		pipeline->vertexInput.attributeDescriptionCnt,
	...
	auto viewportState =
		vkiPipelineViewportStateCreateInfo(1, &viewport, 1, &pipeline->viewport.scissors);

	// rasterization
	auto rasterizationState = vkiPipelineRasterizationStateCreateInfo(
		false,
		false,
		pipeline->rasterization.polygonMode,
		pipeline->rasterization.cullMode,
		pipeline->rasterization.frontFace,
		false,
		0.0,
		0.0,
		0.0,
	...
	blendAttachments[0] = vkiPipelineColorBlendAttachmentState(
		...
		pipeline->blend.srcColorFactor,
		pipeline->blend.dstColorFactor,
		pipeline->blend.colorOp,
		pipeline->blend.srcAlphaFactor,
		pipeline->blend.dstAlphaFactor,
		pipeline->blend.alphaOp,
		colorComponentFlags
	);
	...
	vkCreateGraphicsPipelines(
		device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &ret
	);
